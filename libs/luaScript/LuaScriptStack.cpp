...
LuaScriptStack::LuaScriptStack(TOLUA_OPEN_FUNC funcs[])
{
...
bool LuaScriptStack::executeScriptFile(const string& filename)
{
	int status = lua_status(_state);
	if (status != LUA_OK && status != LUA_ERRRUN)
		return false;
...
void LuaScriptStack::setVariable(const char* varName, const char* varType, void* pData)
{
...
void LuaScriptStack::clearScriptFileName2ContentCache(const string& filename /*= ""*/)
{
	if (filename == "")
	{
