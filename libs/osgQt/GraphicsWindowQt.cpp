...
#define GLWGT_CAST(var) ((GLWidget*)(var))
...
GLWidget::GLWidget(QGLContext* context, QWidget* parent, const QGLWidget* shareWidget, Qt::WindowFlags f, bool forwardKeyEvents)
...
GLWidget::GLWidget(const QGLFormat& format, QWidget* parent, const QGLWidget* shareWidget, Qt::WindowFlags f, bool forwardKeyEvents)
...
GraphicsWindowQt::GraphicsWindowQt(osg::GraphicsContext::Traits* traits, QWidget* parent, const QGLWidget* shareWidget)
	: _realized(false)
{

	_widget = NULL;
	_traits = traits;
	init(parent, shareWidget);
}
...
bool GraphicsWindowQt::init(QWidget* parent, const QGLWidget* shareWidget)
{
	...
	GLWGT_CAST(_widget)->setAutoBufferSwap(false);
	GLWGT_CAST(_widget)->setMouseTracking(true);
	GLWGT_CAST(_widget)->setFocusPolicy(Qt::WheelFocus);
	GLWGT_CAST(_widget)->setGraphicsWindow(this);
...
void GraphicsWindowQt::useCursor(bool cursorOn)
{
	if (_widget)
	{
		...
		else _widget->setCursor(_widget->cursor());
	}
}
...
void GraphicsWindowQt::setCursor(MouseCursor cursor)
{
	...
	switch (cursor)
	{
	case NoCursor: currentCursor = Qt::BlankCursor; break;
	case RightArrowCursor: case LeftArrowCursor: currentCursor = Qt::ArrowCursor; break;
	case InfoCursor: currentCursor = Qt::SizeAllCursor; break;
	case DestroyCursor: currentCursor = Qt::ForbiddenCursor; break;
	case HelpCursor: currentCursor = Qt::WhatsThisCursor; break;
	case CycleCursor: currentCursor = Qt::ForbiddenCursor; break;
	case SprayCursor: currentCursor = Qt::SizeAllCursor; break;
	case WaitCursor: currentCursor = Qt::WaitCursor; break;
	case TextCursor: currentCursor = Qt::IBeamCursor; break;
	case CrosshairCursor: currentCursor = Qt::CrossCursor; break;
	case HandCursor: currentCursor = Qt::OpenHandCursor; break;
	case UpDownCursor: currentCursor = Qt::SizeVerCursor; break;
	case LeftRightCursor: currentCursor = Qt::SizeHorCursor; break;
	case TopSideCursor: case BottomSideCursor: currentCursor = Qt::UpArrowCursor; break;
	case LeftSideCursor: case RightSideCursor: currentCursor = Qt::SizeHorCursor; break;
	case TopLeftCorner: currentCursor = Qt::SizeBDiagCursor; break;
	case TopRightCorner: currentCursor = Qt::SizeFDiagCursor; break;
	case BottomRightCorner: currentCursor = Qt::SizeBDiagCursor; break;
	case BottomLeftCorner: currentCursor = Qt::SizeFDiagCursor; break;
	default: break;
	};
	if (_widget) _widget->setCursor(currentCursor);
}

bool GraphicsWindowQt::getTouchEventsEnabled() const
{
	return GLWGT_CAST(_widget)->getTouchEventsEnabled();
}

void GraphicsWindowQt::setTouchEventsEnabled(bool e)
{
	GLWGT_CAST(_widget)->setTouchEventsEnabled(e);
}
...
void GraphicsWindowQt::runOperations()
{
	...
	if (GLWGT_CAST(_widget)->getNumDeferredEvents() > 0)
		GLWGT_CAST(_widget)->processDeferredEvents();
...
private:
	...
	QtWindowingSystem(const QtWindowingSystem&) = delete;
	QtWindowingSystem& operator=(const QtWindowingSystem&) = delete;
};
