...
public:
	static Spawner* create(int id, int breed, const string& fixedCom, const string& context = "", const string& desc = "");
	...
	class Context : public Type { PROPERTY_R(Spawner*, _spawner, Spawner) friend class Spawner; public: virtual ~Context() {} };
...
private:
	void* _parser;
	string _fixedCom; // 创建实体时必须同时创建的组件,其组件实现的后缀为Impl.
