...
std::string CmdParser::getAppName() const
{
    if (_argc && *_argc>0 )
...
void CmdParser::reportRemainingOptionsAsUnrecognized(ErrorSeverity severity)
{
    ...
    if (_usage)
    {
        ...
        for(AppUsage::UsageMap::const_iterator itr=_usage->getCommandLineOptions().begin();
...
void CmdParser::writeErrorMessages(std::ostream& output,ErrorSeverity severity)
{
        ...
        ++itr)
    {
        ...
        {
            output<< getAppName() << ": " << itr->first << std::endl;
        }
    }
}

AppUsage::Type CmdParser::readHelpType()
{
    getAppUsage()->addCommandLineOption("-h or --help","Display command line parameters");
    getAppUsage()->addCommandLineOption("--help-env","Display environmental variables available");
    getAppUsage()->addCommandLineOption("--help-keys","Display keyboard & mouse bindings available");
    getAppUsage()->addCommandLineOption("--help-all","Display all command line, env vars and keyboard & mouse bindings.");

    // if user request help write it out to cout.
    if (read("--help-all"))             return AppUsage::HELP_ALL;
    if (read("-h") || read("--help"))   return AppUsage::COMMAND_LINE_OPTION;
    if (read("--help-env"))             return AppUsage::ENVIRONMENTAL_VARIABLE;
    if (read("--help-keys"))            return AppUsage::KEYBOARD_MOUSE_BINDING;

    return AppUsage::NO_HELP;
}
