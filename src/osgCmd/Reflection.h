...
#define REFLEX_DECLARE(CLASS_TYPE) private: static osgCmd::Reflex<CLASS_TYPE> _dynReflex;
#define REFLEX_IMPLEMENT(CLASS_TYPE) osgCmd::Reflex<CLASS_TYPE> CLASS_TYPE::_dynReflex

#include <osgCmd/Support.h>

namespace osgCmd {

class _osgCmdExport Type
{
public:
	virtual ~Type() {}
...
public:
	...
	Reflex()
	{
		...
		{
			OSGCMD_ASSERT(false && "ReflexFactory.enroll() has failed!");
		}
	}
	~Reflex()
	{
		...
		{
			OSGCMD_ASSERT(false && "ReflexFactory.unenroll() has failed!");
		}
	}
};
