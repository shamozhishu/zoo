...
string w2a_(const wstring& wideStr)
{
	...
	string ansiStr;
	int needBytes = WideCharToMultiByte(CP_ACP, 0, wideStr.c_str(), (int)wideStr.size(), NULL, 0, NULL, NULL);
	if (needBytes > 0)
	{
		ansiStr.resize(needBytes);
		WideCharToMultiByte(CP_ACP, 0, wideStr.c_str(), (int)wideStr.size(), &ansiStr[0], needBytes, NULL, NULL);
	}
...
wstring a2w_(const string& ansiStr)
{
	...
	wstring wideStr;
	int needWChar = MultiByteToWideChar(CP_ACP, 0, ansiStr.c_str(), (int)ansiStr.size(), NULL, 0);
	if (needWChar > 0)
	{
		wideStr.resize(needWChar);
		MultiByteToWideChar(CP_ACP, 0, ansiStr.c_str(), (int)ansiStr.size(), &wideStr[0], needWChar);
	}
...
bool findFileDir(vector<string>& arrFilePath, const string& strCurPath, const string& fileFormat)
{
	string searchPath = strCurPath + "*";
	_finddata_t sFileInfo;
	intptr_t handle = _findfirst(searchPath.c_str(), &sFileInfo);
	if (-1l == handle)
		return false;
	do
	{
		...
		{
			if (0 != strcmp(sFileInfo.name, ".") && 0 != strcmp(sFileInfo.name, ".."))
			{
				string newPath = strCurPath + sFileInfo.name + "\\";
				findFileDir(arrFilePath, newPath, fileFormat);
			}
			continue;
		}

		string resultPath = strCurPath + sFileInfo.name;
		string strExName = resultPath;
	...
	} while (0 == _findnext(handle, &sFileInfo));
