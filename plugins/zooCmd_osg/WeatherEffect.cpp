...
WeatherEffect::WeatherEffect(OsgContext* context)
	: _osgContext(context)
	, _islonlat(false)
	, _isLocal(false)
...
void WeatherEffect::update()
{
	...
	if (_isLocal)
	{
		OsgEarthContext* osgearthContext = dynamic_cast<OsgEarthContext*>(_osgContext);
		if (osgearthContext)
		{
			...
			{
				osg::Vec3d worldXYZ;
				bool succ = _islonlat ? pOsgEarthUtils->convertLLHToXYZ(osgearthContext, _weatherPos.x(), _weatherPos.y(), _weatherPos.z(), worldXYZ.x(), worldXYZ.y(), worldXYZ.z()) : false;
				if (succ)
				{
	...
	PrecipitationDrawableSet& pds, osg::Polytope& frustum, osgUtil::CullVisitor* cv) const
{
	...
	if (osgearthContext)
	{
		...
		{
			osg::Vec3d vecLL;
			pOsgEarthUtils->convertXYZToLLH(osgearthContext, position.x(), position.y(), position.z(), vecLL.x(), vecLL.y(), vecLL.z());
			lon = vecLL.x();
			lat = vecLL.y();
			hei = vecLL.z();
		}
